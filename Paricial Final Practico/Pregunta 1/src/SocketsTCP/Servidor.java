    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            do {
                ...
                if (Comando.trim().equals("factorial")) {
                    texto = ReturnFactorial(Numeros[0]);
                    System.out.println(" factorial : " + texto);
                }

                if (Comando.equals("fibonaci")) {
                    texto = ReturnFibonacci(Numeros[0]);
                }

                if (Comando.equals("invertir")) {
                    System.out.println("longitud de listacadenascopia : " + ListadeCadenasCopia.size());
//                    System.out.println("Valor adentro de la lsita -0>"+ListadeCadenasCopia.get(0));
                    texto = Invertir(ListadeCadenasCopia);
                }

                if (Comando.equals("oracion")) {
                    texto = Oracion(ListadeCadenasCopia);
                }
    ...
    public static String Invertir(ArrayList Cadena) {
        System.out.println("el sizae :: " + Cadena.size());
        int longitud = Cadena.size();
        if (longitud == 0) {
            return "resultado : no_n";
        } else {
            return "resultado : " + Cadena.get(0);
        }
    }
    ...
    public static String ReturnFactorial(int Mensaje) {
        System.out.println(" Entro al factorial calculando");
        int factorial = Factorial(Mensaje);
        return "resultado : " + factorial;
    }

    public static String ReturnFibonacci(int Mensaje) {
        String respuesta = Fibonacci(Mensaje);
        if (Mensaje > 0) {
    ...
    public static String Oracion(ArrayList Cadena) {
        int longitud = Cadena.size();
        if (longitud == 0) {
            return "resultado : no_n";
        } else {
            return "resultado : " + upperCaseFirst((String) Cadena.get(0));
        }
    }
