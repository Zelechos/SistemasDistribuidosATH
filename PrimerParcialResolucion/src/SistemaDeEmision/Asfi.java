...
public class Asfi extends UnicastRemoteObject implements IConexion {
    ...
    public ArrayList<String> testingServicios() throws RemoteException {
        ...
        //Banco BNB RMI FUNCIONA!!!!
        //----------------------------------------------------------------
        IConexionBNB BNB;
        String response = "";
        try {

            BNB = (IConexionBNB) Naming.lookup("rmi://localhost/BANCOBNB");
            
            System.out.println(BNB.testing());
        ...
        System.out.println("respuesta DE BNB_CLIENTE ->  " + response);
        Bancos.add(response);
        //----------------------------------------------------------------
        
        
        
        //Banco Mercantil Socket TCP    FUNCIONA!!!!
        //----------------------------------------------------------------
...
//        Mensaje += Lista.get(0).CI+"-"+Lista.get(0).Nombres+"-"+Lista.get(0).Apellido;
        ...
        try {
            ...
            try (DatagramSocket socketUDP = new DatagramSocket()) {
                ...
                byte[] Men = resp.getBytes();
                InetAddress hostServidor = InetAddress.getByName(ip);
                // Construimos un datagrama para enviar el mensaje al servidor
                DatagramPacket peticion = new DatagramPacket(Men, resp.length(), hostServidor, puerto);
    ...
    //Cliente RMI BNB
    public static String BNB_Cliente() {
        IConexionBNB BNB;
        String response = "";
        try {
